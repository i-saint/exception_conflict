...
namespace exception {
  ...
  template<class T>
  inline T trim(T min_value, T v, T max_value)
  {
    return std::max<T>(min_value, std::min<T>(max_value, v));
  }

  inline string trim(const string& s, int size)
  {
    if(s.size()<=size) {
      return s;
    }
  ...
  inline void DrawText(const wstring& str, const sgui::Rect& rect, int style=1)
  {
    if(sgui::Font *font = GetFont()) {
      font->draw(str, rect, style);
    }
  }
  ...
  inline int GetLevelFromString(const string& l)
  {
    if     (l=="light")  { return LEVEL_LIGHT;  }
    else if(l=="normal") { return LEVEL_NORMAL; }
    else if(l=="heavy")  { return LEVEL_HEAVY;  }
    else if(l=="excess") { return LEVEL_EXCESS; }
    else if(l=="future") { return LEVEL_FUTURE; }
    return -1;
  }

  inline string GetLevelString(int level)
  {
    if     (level==LEVEL_LIGHT)  { return "light";  }
    else if(level==LEVEL_NORMAL) { return "normal"; }
    else if(level==LEVEL_HEAVY)  { return "heavy";  }
    else if(level==LEVEL_EXCESS) { return "excess"; }
    else if(level==LEVEL_FUTURE) { return "future"; }
    return "";
  }

  inline string GetLevelString()
  {
    if(IGame *g = GetGame()) {
      return GetLevelString(g->getLevel());
    }
    return "";
  }


  inline int GetMapFromString(const string& l)
  {
    if     (l=="horde")        { return MAP_HORDE;  }
    else if(l=="deathmatch")   { return MAP_DEATHMATCH; }
    else if(l=="teamfortress") { return MAP_TEAMFORTRESS;  }
    return -1;
  }

  inline string GetMapString(int map)
  {
    if     (map==MAP_HORDE)        { return "horde";  }
    else if(map==MAP_DEATHMATCH)   { return "deathmatch"; }
    else if(map==MAP_TEAMFORTRESS) { return "teamfortress";  }
    return "";
  }
  ...
  void SerializeContainer(Serializer& s, const Container& v)
  {
    s << v.size();
  ...
  template<class Container>
  void DeserializeContainer(Deserializer& s, Container& v)
  {
    size_t size;
    s >> size;
    for(size_t i=0; i<size; ++i) {
      v.push_back(typename Container::value_type(s));
    }
  }


  template<class Container>
  void SerializePtrContainer(Serializer& s, const Container& v)
  {
    s << v.size();
    for(typename Container::const_iterator i=v.begin(); i!=v.end(); ++i) {
      (*i)->serialize(s);
  ...
  template<class Container>
  void DeserializePtrContainer(Deserializer& s, Container& v)
  {
    size_t size;
    s >> size;
    for(size_t i=0; i<size; ++i) {
      typedef typename Container::value_type::element_type element_type;
      v.push_back(new element_type(s));
    }
  }
  ...
  {
    v = CheckedPopLinkage<typename UnPtr<Ptr>::type>();
  }
  ...
  {
    for(size_t i=0; i<N; ++i) {
      v[i] = CheckedPopLinkage<typename UnPtr<Ptr>::type>();
    }
  }
  ...
  void ReconstructLinkageContainer(T& v)
  {
    for(typename T::iterator i=v.begin(); i!=v.end(); ++i) {
      *i = CheckedPopLinkage<typename UnPtr<typename T::value_type>::type>();
    }
  }
  ...
  player_ptr GetLocalPlayer();
  team_ptr GetLocalTeam();
