...
#pragma region Windows Form Designer generated code
    ...
    void InitializeComponent(void)
    {
      ...
      this->ck_frameskip->Text = L"フレームスキップ";
      ...
      this->cm_dir_y->Items->AddRange(gcnew cli::array< System::Object^  >(1) {L"無効"});
      ...
      this->cm_dir_x->Items->AddRange(gcnew cli::array< System::Object^  >(1) {L"無効"});
      ...
      this->bu_reset_keyassign->Text = L"キーコンフィグ初期化";
...
private: System::Void cm_pad1_MouseEnter(System::Object^  sender, System::EventArgs^  e) {
           Regex^ reg = gcnew Regex(L"^バッティング");
...
private: System::Void checkKeyBatting(System::Object^  sender, System::EventArgs^  e) {
           ...
           for(int i=0; i<9; ++i) {
             for(int j=i+1; j<10; ++j) {
               if(cb[i]->SelectedIndex==cb[j]->SelectedIndex && cb[i]->SelectedItem!=L"無効") {
                 setDescription(L"バッティングしてるキーがあります！: "+cb[i]->SelectedItem);
                 goto loop_end;
               }
             }
           }
...
private: System::Void checkButtonBatting(System::Object^  sender, System::EventArgs^  e) {
           ...
           for(int i=0; i<6; ++i) {
             for(int j=i+1; j<7; ++j) {
               if(cb[i]->SelectedIndex==cb[j]->SelectedIndex && cb[i]->SelectedItem!=L"無効") {
                 setDescription(L"バッティングしてるボタンがあります！: "+cb[i]->SelectedItem);
                 goto loop_end;
               }
             }
           }
...
private: System::Void ck_frameskip_MouseEnter(System::Object^  sender, System::EventArgs^  e) {
           setDescription(L"処理落ちしたとき、描画をスキップしてゲームの進行速度をある程度維持します。");
         }
private: System::Void bu_color_MouseEnter(System::Object^  sender, System::EventArgs^  e) {
           setDescription(L"自機の色を指定します。");
         }
...
private: System::Void bu_reset_keyassign_MouseEnter(System::Object^  sender, System::EventArgs^  e) {
           setDescription(L"キーコンフィグを初期設定に戻します。");
         }
};
