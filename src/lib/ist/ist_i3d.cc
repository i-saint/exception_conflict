...
Box& Box::expand(const vector4& v)
{
  m_ur+=v;
  m_bl-=v;
  return *this;
...
vector4 Box::inner(const vector4& v) const
{
  vector4 r = v;
  const vector4& bl = getBottomLeft();
  const vector4& ur = getUpperRight();
  r.x = std::max<float>(bl.x, std::min(ur.x, r.x));
  r.y = std::max<float>(bl.y, std::min(ur.y, r.y));
  r.z = std::max<float>(bl.z, std::min(ur.z, r.z));
  return r;
}
