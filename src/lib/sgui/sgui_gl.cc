...
float Font::getHeight(const wstring& str) const
{
  return getHeight(str.c_str(), str.size());
}
...
float Font::getAdvance(const wstring& str) const
{
  return getAdvance(str.c_str(), str.size());
}
...
void handleSDLEvent(const SDL_Event& evt)
{
  ...
  case SDL_KEYDOWN:
    // SDLのバグなのか、漢字キーは押しっぱなしにしたときと同様の挙動をするんで状態チェック 
    if(evt.key.keysym.sym==96) {
      if(s_kanji_key) {
        return;
      }
      s_kanji_key = true;
  ...
  m_mouse_focus_window(0)
{
  ...
  if(GetWindowsDirectoryA(path, 256)) {
    ...
    if(GetVersionEx(&osvi) && osvi.dwMajorVersion>=6) { // vista以降の場合meiryo.ttc使用 
      font+="meiryo.ttc";
    }
    else {
      font+="msgothic.ttc";
...
bool App::handleEvent(const Event& evt)
{
  ...
  if(evt.getType()==EVT_APP_DESTROY_WINDOW) {
    ...
    if(src==m_key_focus_window)  { m_key_focus_window=0; }
    if(src==m_mouse_focus_window){ m_mouse_focus_window=0; }
    return true;
  }
