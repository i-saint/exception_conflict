...
#define SGUI_ENABLE_FILEDIALOG // FileDialogに対応する。
#define SGUI_ENABLE_IMEWINDOW  // 日本語入力に対応する。現状Windows限定機能 
...
void SetClipboardString(const wstring& mes);
...
inline wstring Format(const wchar_t *format, ...)
{
  wchar_t buf[256];
  vswprintf(buf, 256, format, (char*)(&format+1));
  return buf;
}
...
//  EVT_RADIOBUTTON,      // Event
  EVT_LIST_MOVE_SELECTION,// ListEvent
  ...
  EVT_COLORDIALOG,        // DialogEvent
  EVT_FILEDIALOG,         // DialogEvent

  EVT_IME_DETERMINE,      // IMEEvent
...
{
public:
  DialogEvent(EventType type, Window *src, Window *dst, bool ok) :
    Event(type, src, dst), m_ok(ok)
  {}
  ...
  DialogEvent(EventType type, Window *src, Window *dst, bool ok, const Color& color) :
    Event(type, src, dst), m_ok(ok), m_color(color)
  {}
...
{
public:
  IMEEvent(EventType type, Window *src, Window *dst, const wstring& str) :
    Event(type, src, dst), m_string(str)
  {}
  const wstring& getString() const { return m_string; }
...
public:
  ...
  float getHeight(const wstring& str) const;
  float getAdvance(const wchar_t *str, size_t length) const;
  float getAdvance(const wstring& str) const;
...
public:
  ...
  Slider(Window *parent, const Point& pos, float size=0.0f, int style=0, wnd_id id=0);
  virtual void draw();
  ...
  virtual void setScrollValue(float value);
...
public:
  ...
  virtual float getStep() const;
  virtual float getRange() const;
  virtual const Color& getBarColor() const;
  virtual Button* getScrollButtonTopLeft() const;
  virtual Button* getScrollButtonBottomRight() const;
  ...
  virtual void setStep(float value);
  virtual void setRange(float range);
  virtual void setBarColor(const Color& color);
...
public:
  ...
  virtual void upSelection();
  virtual void downSelection();
...
typedef Dialog Super;
public:
  ColorDialog(Window *parent, const Point& pos, wnd_id id=0, const wstring& title=L"color", const Size& size=Size(240.0f, 140.0f));
  virtual bool handleEvent(const Event& event);
  virtual Color getColor();
  virtual void setColor(const Color& c);

protected:
  Slider *m_color_slider[3];
  Edit *m_color_edit[3];
  Panel *m_color_sample;
  Button *m_ok_button;
  Button *m_cancel_button;
};
...
#endif // SGUI_ENABLE_FILEDIALOG 



class IMEWindow : public Panel
{
typedef Panel Super;
public:
  static IMEWindow* instance();
...
private:
  ...
  void fitSize();
  void clear();
  void showCandidateList(bool v);

private:
  static IMEWindow *s_inst;
  wstring m_hiragana;
  List *m_candidate_list;
};
